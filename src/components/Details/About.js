    ...
    <div className='w-full h-full lg:col-span-3 mb-4 lg:mb-0'>
      {dataDetails[0].places && dataDetails[0].places.map((item, index) => {
        ...
        <div key={index}>
          ...
          <p className='text-md leading-relaxed text-neutral-400 font-light'>{item.description}</p>
          <div className='w-full h-full mt-6 grid grid-cols-2 sm:grid-cols-4 grid-rows-4 sm:grid-rows-2 gap-4'>
            {Array.from({length: 8}).map((innerItem, innerIndex) => (
              <div key={innerIndex}>
                ...
                <div className='mt-2'>
                  <span className='text-md text-secondary font-medium mr-2'>{item.features[innerIndex]}</span>
                  <span className='text-md text-neutral-400 font-light'>{dataDetails[1].icons[innerIndex].title}</span>
                </div>
              </div>
            ))}
          </div>
