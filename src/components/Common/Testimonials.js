        ...
        {dataDetails[0].places && dataDetails[0].places.map((item, index) => {
          return (item.id === parseInt(id) &&
            <div key={index} className='w-full h-full px-4 md:px-0 flex flex-col md:flex-row justify-start items-center'>
              <div className='relative w-full lg:w-[450px] h-[320px] sm:h-[440px] lg:h-[540px] mr-6 md:mr-0'>
              ...
              <div className='flex flex-col justify-center items-start pl-0 md:pl-16 lg:pl-24 mt-16 lg:mt-0'>
                <h5 className='text-xl text-secondary font-medium mb-6 md:mb-12'>{item.testimonial.group}</h5>
                ...
                <h3 className='max-w-full md:max-w-[700px] text-2xl lg:text-3xl leading-snug lg:leading-normal text-secondary mb-2'>
                  {item.testimonial.text}
                </h3>
                <p className='text-xl text-neutral-300 font-light mb-6 md:mb-12'>{item.testimonial.author}</p>
                <button className='px-8 py-4 bg-primary shadow-buttonShadow rounded-md text-xl lg:text-lg text-white hover:brightness-90 duration-200'>
                  Read Their Story
                </button>
              </div>
            </div>
